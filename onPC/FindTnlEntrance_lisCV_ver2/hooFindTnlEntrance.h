...
TunnelInfos hooFindTnlEntrance(lisCV_Image* src, lisCV_Image* inputImage, lisCV_Image* grayInputImage, CrossingPoints cxPoints,lisCV_Image* colorImage, bool ifDoLaneDetection ) //,int brightness)
{							
...
// grayInputImage - gray scaled original image
	...
	if (ifDoLaneDetection == true){
		...
		for (i=0; i<1000; i++){
			...
			if (theta > 290*pi/180 && theta < 310*pi/180 && abs(rho)<100 ){ //&& rho < 0){			
		...
		for (i=0; i<1000; i++){
			...
			if(theta > 230 && theta < 260*pi/180 && abs(rho) < 100){ //&& rho > 0 ){			
